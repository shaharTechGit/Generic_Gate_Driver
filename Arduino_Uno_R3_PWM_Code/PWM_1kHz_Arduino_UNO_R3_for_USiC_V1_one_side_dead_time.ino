...
// const uint16_t dead_time    = 8;	// 500 ns
const uint16_t dead_time    = 1;
const uint16_t pulse_width  = 7800;    // D9 ON time (~48.75% duty)

const int resetN = 13;  // Reset pin to enable gate driver (active LOW)

void setup() {
  // Set output pins
  pinMode(9, OUTPUT);   // OC1A (D9)
  pinMode(10, OUTPUT);  // OC1B (D10)
  pinMode(resetN, OUTPUT); // Reset signal

  // Set reset HIGH to enable the board
...
// Optional: call this function to reset the Eval board manually
void resetEvalBoard() {
